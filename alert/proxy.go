...
func (s *alertKubernetesProxy) SendDocumentToAlertGroup(ctx context.Context, chatid uint32, document []byte, extension string) error {
	docAlert := makeAlertSendDocumentToAlertGroupHTTPProxy(s.namespace, chatid, extension, s.logger)
	docAlert = circuitbreaker.Gobreaker(gobreaker.NewCircuitBreaker(gobreaker.Settings{}))(docAlert)
	docAlert = ratelimit.NewErroringLimiter(rate.NewLimiter(rate.Every(time.Second), 10))(docAlert)
	_, err := docAlert(context.WithValue(getContext(ctx), "CHAT-ID", chatid), docAlert)
	return err
...
func makeAlertSendDocumentToAlertGroupHTTPProxy(namespace string, chatId uint32, documentExtension string, logger log.Logger) endpoint.Endpoint {
	return http.NewClient(
		"POST",
		GetURL(namespace, fmt.Sprintf("alert/%v/document/%v", chatId, documentExtension)),
		gokit.EncodeToBase64,
		gokit.DecodeResponse,
		gokit.GetClientOpts(logger)...,
	).Endpoint()
